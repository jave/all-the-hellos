(ns simple-test.integration-test
...
(defn an-once-fixture  [block]
  (println "the once integration fixture is being run!")
...
(defn an-each-fixture [block]
  (do 
    (println "the each integration fixture is being run!")
    (block)
    (println "the each integration fixture is being ended!")
    )
  )
...
;; (deftest a-test
;;   (testing "a test."
;;     (println "the a-test not iintegration marked is being run!")
;;     (is (= 1 1))))
...
(deftest   ^:integration a-integration-test
  ...
  (testing "an integration test."
    (println "the a-inegration-test integration tagger is being run!")
    (is (= 1 1))))
