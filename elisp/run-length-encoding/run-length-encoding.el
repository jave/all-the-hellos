...
;;; Commentary:
;; I wanted to use s.el, and peg.el in this solution, because I was interested in the libraries
;; Sadly you can't include libraries as you please for the exercism test runner to work.
;; So, I just pasted peg.el, and a definition from s.el in her
;; This is hardly optimal, but anyhow...
...
;; this is an uninteresting imperative solution for run-length encoding
;; it uses mapcar for side effects, which is not good practice
(defun run-length-encode(in)
  (let ((last 0)
        (count 0)
        (ack "")
        (in (concat "" in "." )))
...
;;from peg.el:s tests. I think this should be part of peg.el, but it isnt
...
;; I wanted to use peg.el for this, which is fun, but it becomes
;; slightly hard to read because its not always clear what the peg
;; stack is at a particular moment in parsing

(defun run-length-decode (run-length-encoded-string)
  (apply 'concat   ;; concat the list of runs ("www" "bbb") or whatever has been parsed
         (reverse ;; the list comes out in reverse order
          (peg-parse-string
           ((top (or runs empty)) ;; either we have a list of runs, or the empty string at the top level
            (runs (+ run )) ;; runs are one or more run, a run is a string like "www"
            (run (or long-run short-run) `(lengt-char-pair -- (s-repeat (car lengt-char-pair) (cadr lengt-char-pair))))
            ;; a run is a long run or a short run, both place pairs like (2 "w") on the stack
            (long-run (list (and num short-run)))
            (short-run  alpha)
            (num (substring (* [0-9])) `(string -- (if (equal "" string) 1 (string-to-number string))))
            (alpha (substring  (or [A-Z a-z " "] )))
            (empty (null) `( -- "")) ;; "" is a valid rle, and results in ""
            )
